...
if (@ARGV != 1) {
	printf STDERR "nucleotide <nucleic acid pdb file>\n";
	exit;
}
#check model pdb file
if (not -f $ARGV[0]) {
	printf "nucleotide error: pdb file $ARGV[0] doesn't exist\n";
	exit;
}
else {
	#read model pdb
...
for ($i = 0; $i < $nres; $i++) {
	...
	printf  "$rnam[$ratm_star[$i]]  ";
}
printf "\n\n";
printf "total number of nucleotides: $nres\n";
